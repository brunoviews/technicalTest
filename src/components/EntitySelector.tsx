...
export default function EntitySelector({ entity, options, selectedId, onChange }: EntitySelectorProps) {
  ...
  return (
    <div className="relative mb-6 w-full md:w-80">
      <label className="block mb-2 text-base font-semibold text-gray-700">Select {entity}:</label>
      <button
        ref={buttonRef}
        type="button"
          ...
          {options.map(option => (
            ...
            >
              ...
              {selectedId === option.id && (
                <svg className="w-4 h-4 ml-2 text-blue-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
              )}
            </li>
          ))}
        </ul>
